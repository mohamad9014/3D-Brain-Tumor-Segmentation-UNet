...
def unet_model_3d(loss_function, input_shape=(4, 160, 160, 16),
    ...
    for layer_depth in range(depth - 2, -1, -1):
        current_filters = K.int_shape(current_layer)[1]

        up_convolution = get_up_convolution(
            pool_size=pool_size,
            deconvolution=deconvolution,
            n_filters=current_filters
        )(current_layer)

        concat = concatenate([up_convolution, levels[layer_depth][1]], axis=1)

        skip_filters = K.int_shape(levels[layer_depth][1])[1]

        current_layer = create_convolution_block(
            n_filters=skip_filters,
            input_layer=concat,
            batch_normalization=batch_normalization
        )
...
def predict_and_viz(image, label, model, threshold, loc=(100, 100, 50)):
    ...
    for x in range(0, image.shape[0], 160):
        for y in range(0, image.shape[1], 160):
            for z in range(0, image.shape[2], 16):
                ...
                pred = model.predict(np.expand_dims(patch, 0), verbose=0)
    ...
    model_labeled_image = get_labeled_image(
        image, model_label_reformatted, is_categorical=True
    )
